...
public class DatabaseManager {
	...
	/**
	 * XML로딩 database-config.xml을 읽어서 database 객체의 property들을 채운다.
	 * @param configXml
	...
	private void loadConfig(String configXml) throws SqlExecutorException{
		 ...
		 if(is == null){
			 System.out.println(configXml+ " loading error");
		 }
		 // File xmlFile = new File(configXml);
		  ...
		  try {
			...
			List dbList = rootNode.getChildren("database");
			for (int i = 0; i < dbList.size(); i++) {
			   Database database = new Database();
			   
			   Element dbElement = (Element) dbList.get(i);
			   ...
			   List listSqlFiles = e.getChildren();
			   for(int j=0;j<listSqlFiles.size();j++){
				   Element sqlElement = (Element) listSqlFiles.get(j);
